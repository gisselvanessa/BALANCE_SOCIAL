...
@Component({
  selector: 'app-principle2',
  templateUrl: './principle2.component.html',
  styleUrls: ['./principle2.component.scss']
})
export class Principle2Component {
  ...
  async getDataInfo(page: number, pageSize: number): Promise<void> {
    try {
      this.loading = true
      const data: any = await this.indicatorService.getDataById('2', this.searchText).toPromise();
  ...
  saveAim(row_obj: any): boolean | any {
    ...
    if (row_obj.operacion === 'División' || row_obj.operacion === 'Igual' || row_obj.operacion === 'División - 1') {
  ...
  openDialog(action: string, obj: ObjectiveModel): void {
    ...
    const dialogRef = this.dialog.open(DialogPrinciple2Component, {
      data: {
        param1: obj,
...
@Component({
  ...
  selector: 'app-dialog-principle2',
  templateUrl: './dialog-principle2.html'
})
// tslint:disable-next-line: component-class-suffix
export class DialogPrinciple2Component implements OnInit {
  action: string;
  ...
  constructor(
    public datePipe: DatePipe,
    public dialogRef: MatDialogRef<DialogPrinciple2Component>,
