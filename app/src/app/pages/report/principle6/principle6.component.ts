...
@Component({
  selector: 'app-principle6',
  templateUrl: './principle6.component.html',
  styleUrls: ['./principle6.component.scss']
})
export class Principle6Component {
  ...
  async getDataInfo(page: number, pageSize: number): Promise<void> {
    try {
      this.loading = true
      const data: any = await this.indicatorService.getDataById('6', this.searchText).toPromise();
  ...
  openDialog(action: string, obj: ObjectiveModel): void {
    ...
    const dialogRef = this.dialog.open(DialogPrinciple6Component, {
      data: {
        param1: obj,
...
@Component({
  ...
  selector: 'app-dialog-principle6',
  templateUrl: './dialog-principle6.html'
})
// tslint:disable-next-line: component-class-suffix
export class DialogPrinciple6Component implements OnInit {
  action: string;
  ...
  constructor(
    public datePipe: DatePipe,
    public dialogRef: MatDialogRef<DialogPrinciple6Component>,
    ...
    this.local_data_objective.isAverage = data.param1.relacionproporcion === "Porcentaje" || data.param1.relacionproporcion === "Porcentaje_Division" ? false : data.param1.relacionproporcion === "Promedio_Procentaje" ? false:true;
